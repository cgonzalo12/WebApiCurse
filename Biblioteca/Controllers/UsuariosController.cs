...
namespace Biblioteca.Controllers
{
    ...
    public class UsuariosController: ControllerBase
    {
        ...
        {
            var ususario = await serviciosUsuarios.ObtenerUsuario();

            if (ususario is null)
            {
                return NotFound();
            }
            var credencialesUsuariosDTO = new CredencialesUsuarioDTO { Email = ususario.Email! };
        ...
        public async Task<ActionResult> Put(ActualizarUsuarioDTO actualizarUsuarioDTO)
        {
            ...
            Console.WriteLine($"******Fecha de nacimiento actual del usuario: {usuario.FechaNacimiento}");

            usuario.FechaNacimiento = actualizarUsuarioDTO.FechaNacimiento;

            Console.WriteLine($"Nueva fecha de nacimiento proporcionada: {actualizarUsuarioDTO.FechaNacimiento}");

            var result = await userManager.UpdateAsync(usuario);

            if (result.Succeeded)
            {
                Console.WriteLine($"Fecha de nacimiento actualizada en la base de datos: {usuario.FechaNacimiento}");
                return NoContent();
            }
            else
            {
                ...
                {
                    Console.WriteLine($"Error al actualizar la fecha de nacimiento: {error.Description}");
                    ModelState.AddModelError(string.Empty, error.Description);
                }
                return BadRequest(ModelState); // Es mejor devolver BadRequest con ModelState en caso de error
            }
        }
